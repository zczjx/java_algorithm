...
class SymTlb_RB_Tree<Key extends Comparable<Key>, Val> 
{
...
public class Concordance{
	...
	public static void main(String args[]){
		...
		String[] wd = myin.readAllStrings();
		SymTlb_RB_Tree<String, My_set<Integer>> st = new SymTlb_RB_Tree<String, My_set<Integer>>();
		//init stat set
		for(int i = 0; i < wd.length; i++){
			String s = wd[i];
			if(!st.contains(s))
				st.put(s, new My_set<Integer>());
			My_set<Integer> set = st.get(s);
			set.add(i);
		}
		StdOut.println("Finished init stat!!");
		while(!StdIn.isEmpty()){
			String query = StdIn.readString();
			My_set<Integer> set = st.get(query);
			if(set != null){
				int j = set.min();
				StdOut.println(wd[j] + " : ");
				for(int k : set)
