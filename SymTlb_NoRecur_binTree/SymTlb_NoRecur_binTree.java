...
public class SymTlb_NoRecur_binTree<Key extends Comparable<Key>, Val> 
{
	private BinNode<Key, Val> root;

	
	public SymTlb_NoRecur_binTree(){
		this.root = null;
	...
	private BinNode<Key, Val> put(BinNode<Key, Val> node, Key k, Val v){
		...
		BinNode<Key, Val> tmp = new BinNode<Key, Val>(k, v, 1);
		BinNode<Key, Val> parent = null;
		BinNode<Key, Val> x = node;
		while(x != null){
			parent = x;
			int cmp1 = k.compareTo(x.k);
			if(cmp1 < 0)
				x = x.left;
			else if(cmp1 > 0)
				x = x.right;
		...
		int cmp2 = k.compareTo(parent.k);
		if(cmp2 < 0)
			parent.left = tmp;
		else
			parent.right = tmp;
		BinNode<Key, Val> y = node;
		while(y != null){
			int cmp3 = k.compareTo(y.k);
			if(cmp3 < 0){
				y.node_nr++;
				y = y.left;
			}
	...
	public static void main(String args[]){
		SymTlb_NoRecur_binTree<String, Double> gpa = new SymTlb_NoRecur_binTree<String, Double>();
		gpa.put("C",  2.00);
		gpa.put("C+", 2.33);
		gpa.put("D",  1.00);
		gpa.put("B",  3.00);
